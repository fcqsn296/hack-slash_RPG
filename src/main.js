...
(function (RPG) {
   ...
   *
   * ── 見えなくするだけでは足りなかった ──
   * 上の修正で**重なりは消えたが、鍵盤は地図に届いたまま**だった。
   * 地図の操作は `window` の keydown に付いているので、隠しても外れない。
   * 実際、拠点で装備を触っているつもりで矢印キーを押すと、映っていない地図の上を
   * 主人公が歩き、遭遇して戦闘へ叩き込まれる。Enter を押せば階段が発動して
   * 別のマップへ移動していた（mp_vault → mp_deep を実測）。
   *
   * 呼び忘れた場所を1つ直すのではなく、**画面を切り替える一箇所で外す**。
   * 地図以外を出すなら、地図の操作は必ず外れる——呼び忘れの余地がなくなる。
   *
   * @param {string} id
   */
  ...
  function showMap(mapId, at) {
    ...
    //
    // 帯を先に直しておく。ここで抜けると下の refreshTopbar に届かないので、
    // **最初のシーンを読んでいるあいだだけ周回側の所持金が出ていた**。
    // データは分かれているので表示だけの話だが、モードが漏れているように見える。
    if (playPendingScene()) {
      refreshTopbar();
      return;
    }
