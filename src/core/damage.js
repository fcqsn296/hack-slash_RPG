...
(function (RPG) {
  ...
  function calc(params) {
    ...
    // 異相「○○を拒む相」(§22)。**その属性だけ**を等倍へ均す。
    //
    // elementNull と同じ位置に置くのは、適応・極意・貫通・双極・心得を
    // まとめて無効にするため。**適応2を抜け道にしないのが要点**で、
    // そこを通すと万能型が素通りして「組み替える理由」が消える。
    //
    // 判定に使うのは attackElement（変換・混沌を通したあとの実効属性）。
    // skill.element で見ると、属性変換で闇にしている攻撃を取りこぼす。
    if (options.denyElement && attackElement === options.denyElement) element = NEUTRAL;
