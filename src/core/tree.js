...
(function (RPG) {
  ...
  const KNOWN_EFFECT_KINDS = [
    ...
    'as_magi', 'weave_gift', 'no_repeat', 'decree', 'bond_power', 'bond_share', 'temperance', 'justice', 'moon', 'wheel', 'self_bind', 'solo_power', 'smite', 'stable_damage', 'stealth', 'support_stack',
  ...
  function effectsOf(defs, levels) {
    ...
    const passives = {
      ...
      decree: 0,           // 手番を味方に渡す (§21 皇帝)。渡せる相手が居る間は本人の技を使えない
    ...
    for (const n of defs) {
      ...
      for (const e of n.effects) {
        ...
        switch (e.kind) {
          ...
          case 'decree': passives.decree = Math.max(passives.decree, amount); break;
