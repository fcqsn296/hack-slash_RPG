...
(function (RPG) {
   ...
   * 上限をどこまで伸ばせるか。
   *
   * **配れる報酬の総量より必ず大きくしておくこと。** ここが小さいと、
   * 後から手に入る報酬が黙って消える。実際に 60 だった頃、
   * クエストで先に 30 を取ったあと、塔の報酬 30 のうち 20 が捨てられ、
   * 画面には「上限 +10（60）」と出るのに数字が動かない、という状態になっていた。
   * この関係は test/tests.js が不変条件として見張っている。
   */
  const MAX_CAP = 100;
