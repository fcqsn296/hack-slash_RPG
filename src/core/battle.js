...
(function (RPG) {
  ...
  // 棘より高いのは、反射の性格を闘技場でも残すため。
  // 両方を同じ倍率にすると、桁違いの相手ではどちらも上限に張り付いて
  // まったく同じ数字になり、「重い一撃に強い」という差が消える。
  //
  // ── 40 では常時張り付いていた (§5.20) ──
  // 闘技場の主(ハード)で実測すると、**返り値が毎回きっちり 1,061,320
  // （26,533 × 40）**だった。1発の重さも `REFLECT_LEVEL_RATE` も
  // 何も効いておらず、上限がそのまま火力になっていた。
  // つまり「重い一撃ほど痛い」という性格が消えていた。
  //
  // ── なぜ 400 か ──
  // 狙われやすさを限界まで積んだ4人編成での実測。
  //   40  … 毎回上限に張り付き、2.9M/ラウンド
  //   400 … **上限が外れ、素の値 490万〜840万が出る**。20.0M/ラウンド・10R
  // 棘(300 → 22.2M/9R)のすぐ後ろに並ぶ位置。
  //
  // 400 では上限が当たらないので、天井は安全弁として残るだけになる。
  // 「1発が極端に重い相手」に対してだけ効く、という本来の役目に戻った。
  //
  // 通常フィールドには影響しない。あちらは素の返りが40万程度で、
  ...
  const STATUS_ON_HIT_RATIO = {
    poison: 0.015,    // 最大HPの1.5%／ラウンド
    // 火傷は相手が動くたびに入るので、1回ぶんは毒の1/3にしておく。
    // 闘技場のボス（3〜5手番）で、ようやく毒1回ぶんと釣り合う。
    burn: 0.005,      // 攻撃するたびに最大HPの0.5%
   ...
   *
   * ── 削る異常（毒・火傷）の上限は「安全弁」であって調整つまみではない ──
   * 一度これを常用域まで下げて失敗した。上限 0.06 に対して疫病の坩堝の素が 0.10
   * だったため、**投資が1点も結果に出ない**（0SP で上限に当たる）状態になった。
   * 上限が実質の値になると、素の値も statusPower も死ぬ。
   *
   * いまは逆向きに置いてある。**素の値を「何も振っていないとき」の値に取り、
   * 全部振った (statusPower 3.7 → ×4.7) ところが狙いの強さ**になるようにした。
   * 上限はそのさらに上、実際の構成では当たらない位置に置く。
   *
   *   毒   素0.015 → 全振り0.070（上限0.10）  1回/ラウンド → 7%/R
   *   火傷 素0.005 → 全振り0.024（上限0.035） 闘技場3回/R  → 7%/R
   *
   * 合計14%/ラウンドで、攻撃型（241SP・7R）と並ぶ。毒と火傷は足し合わせるので、
   * **片方だけ見て決めると必ず倍になる**（一度そう外して4Rになった）。
   *
  ...
  function start(config) {
    ...
    // 依頼の規則「主人公が勅命を使える」(rules.heroDecree、§21 皇帝の解放)。
    //
    // **「主人公を一度も行動させない」(idleHero) だけでは成立しなかった。**
    // このゲームには待機やパスが無いので、主人公の手番が来たら動くしかない。
    // 主人公を最後尾に置き、手番が回る前に倒しきる——という、
    // 説明文（誰にいつ振るわせるかだけを決める）と離れた解き方しか残っていなかった。
    // 依頼のあいだだけ勅命を持たせれば、主人公は手番で仲間に命じ、自ら剣を抜かずに済む。
    // 札そのものは要らない（解放前の依頼なので）。ユニットは戦闘ごとに組み立てるので、
    // ここで passives を書き換えても育成には残らない。
    if (quest && quest.rules && quest.rules.heroDecree) {
      for (const u of battle.party) {
        if (u.id === 'ch_hero' && u.passives) u.passives.decree = Math.max(u.passives.decree || 0, 1);
      }
    }
  ...
  function nextWave(battle) {
    ...
    // ── 「吊るされた男」の負債もここで配り直す ──
    //
    // **一度これを外したが、外す判断が誤りだった。**
    // 外したときの理屈は「毎ラウンド1つ失う負債は返しきれない」。
    // その根拠にした実測が、**返済へ振っていないビルドしか見ていなかった**。
    //
    // 負債は溜まらない。毎ラウンド 1 に**置き直される**（加算ではない）ので、
    // 各ラウンドの代償はちょうど手番1つぶん。払い口は3つあり、
    // ③の自前の再行動（extraActionRate）だけは**毎ラウンド判定される**。
    //
    ...
    //
    // 返せる。ただし**返済を自分で背負うビルドを要求する**——
    // それがこの札の設計そのもの（§21 の「支払い方を選べる負債」）。
    //
    // @知見: 手番の負債は溜まらない。毎ラウンド1に置き直されるので、代償はちょうど手番1つぶん
    // @知見: 代償の重さを測るときは、それを返すビルドも測る。振っていない編成だけ見ると「返せない」と読み違える
    for (const u of battle.party) {
      if (!u.alive) continue;
      u.extraActions = 0;
      u.turnDebt = (u.passives && u.passives.turnDebt) || 0;
  ...
  function performAction(battle, actor, skillId, targets, opts) {
    ...
    if (battle.rules && battle.rules.idleHero && actor.id === 'ch_hero' && !battle.decree) {
      failQuest(battle, battle.rules.heroDecree
        ? '主人公が自ら動いた（命じられる仲間がいなかった）'
        : '主人公が自ら動いた（座して統べる条件を満たせなかった）');
      return false;
    }
