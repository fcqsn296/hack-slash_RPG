 ...
 *
 *   @知見: 終盤ビルドは1ウェーブ1ラウンドの「床」で勝つので、そこでは火力の利が測れない
 *
 *   HP×35 でも 10ウェーブ 10.9R（1ウェーブ1.23R）＝ほぼ床。床より速くはなれないので、
 *   **火力を足しても何も買えず、手番を削る代償だけが残る。**
 *   この土俵で測ると、どんな利を乗せても「害しかない札」に見える
 *   （吊るされた男で3回結論を取り違えた。docs/作業ログ/吊るされた男の代償.md）。
 *   火力の効き目を見たいなら、先に床から離れていることを確かめること:
 *
 *     HP×35 → 1.23R/波（床）  HP×100 → 1.48  HP×300 → 2.04  HP×1000 → 3.29
 *
 *   ついでに、{ hp: 100000 } のような極端な値も使えない。割合ダメージの技が
 *   跳ね上がって順序が乱数で入れ替わる（1手番1,500M という値が出た）。
 * @param {number} [o.seed]
 ...
 * 比較に使うと結論が丸ごと変わる。ツリー技の「黄昏」（威力520）と
