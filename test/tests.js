  ...
  function run() {
    ...
    /* ===== 特殊パッシブ（状況依存のダメージ補正） ===== */
    {
      ...
      // ここで見るのは「届いているか」と「代償が本当に痛いか」の2つ。
      {
        ...
        //   黄昏(520)  2.84倍  **超えて削られた** → cap_break 追加後は 3.10倍
        {
    ...
    /* ===== クエスト (§10.3) ===== */
    {
      ...
      // --- どのフィールドにも依頼が1件はあるか (§10.3) ---
      //
      // フィールドを増やしたときに依頼を足し忘れると、その帯だけ
      // 「回るだけの場所」になる。実際に封絶の浅層(20)・封絶区画(130)・
      // 還らぬ位相(250)の3つが長く空いていた（拡張ロードマップ D1/D2 で埋めた）。
      //
      // いまは全12フィールドが埋まっているので、**空きが出たら落ちる**検査にしてある。
      // 新しいフィールドを足すときは依頼も一緒に考えること。
      {
        ...
        assertTrue('クエスト: どのフィールドにも依頼がある',
          noQuest.length === 0,
          noQuest.length
            ? '空いている: ' + noQuest.map((/** @type {string} */ f) => RPG.data.fields[f].name).join('、')
            : `${Object.keys(RPG.data.fields).length} フィールドすべてに依頼がある`);
      }
    ...
    // ---------------------------------------------------------------
    {
      ...
      // ── この検査は「人数」しか見ていない (§17) ──
      //
      // 「4人いれば4発は出るから、2発の吸収は抜ける」という前提で書いてある。
      // **その前提は成り立たない場合がある。**
      //
      // 実際に詰む編成が見つかった:
      //   累撃型×2（1手番1発）＋ 殴らない2人（盾役・バッファー）
      //   → 1ラウンドに2発しか出ず、2発とも吸われて **本体へ0ダメージ**。
      //   実測で通常もハードも 0/6。多段型×2 に替えれば 6/6 で抜ける。
      //
      // 仕掛けそのものは設計どおりで、闘技場は
      // 「特定のビルドを名指しで否定する」場として作ってある（data/arena.js）。
      // 抜け道もある——3人目を殴らせる、多段を1人入れる、
      // 号令（手番を配る技）を持たせる、のいずれか。号令は実測で 6/6 に戻る。
      //
      // だから仕掛けは変えない。**変えるのはこの検査の言い分のほう。**
      // 人数で安全を主張すると、成り立たない安全を保証したことになる。
      assertTrue('闘技場: 吸収発数が、殴る人数より少ない',
        defs.every((/** @type {any} */ d) => !(d.gimmicks || {}).hitAbsorb
          || d.gimmicks.hitAbsorb < save.party.length),
        `パーティ ${save.party.length} 人。`
        + '※全員が1手番1発しか出さない編成では、この条件でも抜けられない');
