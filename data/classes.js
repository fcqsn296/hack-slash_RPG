 ...
 *
 * ── なぜ条件を付けたか ──
 * クラスはこのゲームで**いちばん大きな一度きりの選択**なのに、
 * 就任が無条件・無料で、レベルとまったく結び付いていなかった。
 * 始めた直後に、何も分からないまま6つから1つを選ばされていた。
 *
 * Lv30 は、スキルツリーの上級ティアが開き、装備枠が3つとも揃ったあたり。
 * 「自分がどう戦いたいか」が見えてから選べる位置に置いてある。
 */
RPG.data.classUnlockLevel = 30;
...
RPG.data.classes = {
  ...
  cls_guardian: {
    ...
    nodes: [
      ...
      {
        // 障壁の厚み (§9.1)。守護者に本丸を置く——大盾の宣誓も障壁の再生も
        // ここにあるので、**積む先が一箇所にまとまる**。
        // ── なぜ派生でなく幹に置くのか ──
        // 城塞派生（攻めを捨てて抜かせない）に置くのが筋に見えるが、
        // **派生ごとの重さが揃わなくなる**（検査が hi-lo<=6 で止める）。
        // 派生に足すと「とりあえず重いほう」を選ぶ理由が生まれてしまう。
        id: 'gd_thick', name: '厚き盾', cost: 2, maxLevel: 3,
        effects: [{ kind: 'barrier_power', value: 0.40 }],
        desc: '自分が張る障壁が 40% 厚くなる',
      },
      {
