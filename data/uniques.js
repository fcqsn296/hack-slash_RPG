...
RPG.data.uniqueEquips = {
  ...
  // ── ロッドだけユニークが無かった (§7.8) ──
  //
  // 12種の装備ベースのうち、**ユニークが1つも無いのはロッドだけ**だった。
  // 魔術の武器はもう1本（魔導書）に『初手の理』があるので、そこと役が
  // ぶつからないものを選ぶ。あちらは「1ラウンド目に全部乗せて、あとは減る」型。
  //
  // ── なぜ「属性を散らす」なのか ──
  // 終盤の編成を実測したところ、**属性を1色に揃えた編成が一貫して速い**
  // （素直な攻撃編成に対して所要 -13〜18%）。揃えるほうが強いので、
  // 散らす理由がどこにも無い状態だった。
  // rainbowPower はツリーに1ノード（tr_rainbow・12SPで+40%）あるだけで、
  // 装備側の受け皿が無い。ここをその受け皿にする。
  //
  // ── 常時最適品にしないための条件 ──
  // rainbowPower は**生きている味方全員の属性が別々のとき**しか乗らない
  // （battle.js）。1人でも被れば0。倒れると判定がやり直されるので、
  // 全員生存を保つ意味も生まれる。
  // ユニークは系統タグ倍率を持たないので、条件を満たせない編成では
  // 竜の箱の武器に素直に負ける。
  uq_prism_rod: {
    name: '七色の杖', base: 'eq_rod', color: '#9ad8c8',
    desc: '一色に染まらないための杖。同じ色がふたつ並ぶと、途端に光らなくなる。',
    // ── 値の決め方（実測） ──
    // 竜の箱の最良ロッド（+9）と入れ替えて、同じキャラの最大火力を比べた。
    //   条件なし          -36.6%   ← ユニークは系統タグを持たないぶん素で負ける
    //   属性が別々のみ     +14%
    //   属性が別々＋撃ち分け +37%
    // 最初 0.45/0.18 で置いたが、両方そろえて +8.5% にしかならず、
    // **パーティ編成ごと縛る条件の見返りとして薄すぎた**。
    // 終盤の実測では1色に揃えた編成のほうが所要 13〜18% 短いので、
    // 散らす側はそれを覆せるだけの見返りが要る。
    // 効果は装備した本人にしか乗らない（attacker.passives を見るため）。
    stats: { magi_power: 190 },
    effects: { rainbowPower: 0.80, varietyPower: 0.20 },
    note: 'パーティ全員の属性が別々なら火力 +80%／直前と違う技を使うと火力 +20%',
  },
};
