...
RPG.data.skills = {
  ...
  // ── 段の出口「威力以外」の一号 (§5.10) ──
  //
  // 段を使う技はこれ以前5本あったが、**全部が攻撃技**だった。
  // どんな消費の仕方をしても行き先が威力なので、支援役とタンクには
  // 段を使う理由が一つも無かった。ここが最初の別の出口になる。
  //
  // クラス技は解禁2〜4・再使用2〜6ラウンドなのに、推奨レベルの戦闘は
  // 実測 1.0〜3.5 ラウンドしか続かない。**2回目が来ることはまず無い。**
  // 段で刻を巻き戻せれば、抱えたまま終わっていた技がもう一度撃てる。
  //
  // 歯止めは cooldown 2 の1つだけでよい。再行動は MAX_EXTRA_ACTIONS に
  // 乗るので、段さえあれば動き続ける、という形にはならない。
  sk_combo_reprise: {
    name: '継ぎ手', kind: 'active', plugin: null,
    scaling_stat: 'atk', damage_type: 'reli', element: 'none',
    power: 0, cooldown: 2,
    params: { combo: { spend: 2, cool: 1, act: true } },
    desc: '段を2つ消費する。消費1段につき味方全員のクールタイムが1ラウンド進み、'
      + '使ったあと自分はもう一度行動できる。攻撃はしない。',
  },
  ...
  //
  // 刻みを置くと、段が通過するたびに別の行が増える。10段で撃てば4行とも乗る。
  // 溜める判断が「どの行まで届かせるか」になり、初めて選択になる。
  //
  // 積み重ねであって置き換えではない。上の段だけが効く形にすると、
  // 途中の刻みが飾りになり、結局いちばん上を狙うだけの一本道に戻る。
  //
  // 威力0なのは、これが**支援の技**だから。段を使う技は長らく全部が攻撃技で、
  // 支援役とタンクには段を使う理由が一つも無かった（継ぎ手がその1本目）。
  sk_combo_ledger: {
    name: '積み上げの証', kind: 'active', plugin: null,
    scaling_stat: 'atk', damage_type: 'reli', element: 'none',
    power: 0, cooldown: 3,
    // needs を最初の刻みに揃える。**無いと、1段で撃って段を捨てられる**
    // （spendAll なので払うだけ払って1行も出ない）。
    params: { combo: { spendAll: true, needs: 2, tiers: [
      { at: 2, shield: 0.12 },
      { at: 4, cool: 1 },
      { at: 7, shield: 0.18 },
      { at: 10, act: true },
    ] } },
    desc: '持っている段を全部使う。2段で味方全員に障壁、4段で刻が1ラウンド戻り、'
      + '7段でさらに障壁、10段で自分がもう一度行動できる。'
      + '届いた段までの効果がすべて重なる。攻撃はしない。',
  },

  // ── 染色 (§9.1) ──
  //
  // 攻撃側の属性操作はツリーが埋め尽くしている（変換・双極・適応・極意・貫通）が、
  // **受け側の属性に触る手段は1つも無かった**。ここが構造的に空いている。
  //
  // 属性特化ビルドは「刺さらないフィールド」で腐るという弱点を抱えている。
  // 染色があれば、その弱点を**連れて行く仲間で補える**。
  //
  // 単体・短ターンにしてある。全体化すると属性特化がそのまま万能になり、
  // 「どの1体をいつ染めるか」という判断が消える。
  //
  // 属性を省略してあるので、**本人の属性が有利を取れる色**に染まる。
  // 本人の属性そのものではない——火で火を殴っても有利は取れない（実際に踏んだ）。
  sk_tree_dye: {
    name: '色を差す', kind: 'active', plugin: 'dye',
    scaling_stat: 'magi_power', damage_type: 'magi', element: 'none',
    power: 90, crit_rate: 0.05,
    params: { turns: 2 },
    desc: '敵1体を殴り、2ターンのあいだ自分の属性に染める。'
      + '自分が有利を取れる色に染まるので、刺さらない相手にも押し通せる。' +
      '相手の攻撃は変わらない。',
  },
