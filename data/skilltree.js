...
RPG.data.skillTree = [
  ...
  {
    // 双撃の理の真裏。**取ると多段が一切出なくなる**代わりに、撃つたびに倍。
    //
    // ── なぜ代償も排他フラグも要らないのか ──
    // 「1発を強くする投資」は、これまでどれも多段が上から掛け算していた
    // （大技の威力・上限突破・会心倍率・超過の残存率——全部）。
    // だから単発型は多段型の下位互換にしかならず、代償をいくら重くしても
    // 選択にならなかった（作業ログ「多段と一撃の釣り合い」に一連の実測がある）。
    //
    // この節は **仕掛け自身が多段を殺す** ので、掛け算の相手がいない。
    // 取った時点で別の型になる。
    //
    // ── 何と噛み合うか ──
    // 伸びるのは「1手あたりの回数」ではなく **手番の数**。
    // 再行動・奇襲・手番の前借りに、初めて明確な使い道が出る。
    //
    // ── 立ち上がりが遅いのは上限のおかげ ──
    // 倍々にしても超過分は0.1倍に潰されるので、序盤は伸びが鈍い
    // （前段比 1.22 → 1.36 → 1.53 → 1.82 と徐々に2倍へ近づく）。
    // 助走を作り込まなくても、既にある上限がその役をしている。
    id: 'tr_escalate', tier: 'high', name: '累撃の理', cost: 8, maxLevel: 1,
    effects: [{ kind: 'escalate', value: 2 }],
    desc: '攻撃技が1回しか発動しなくなる代わりに、撃つたびに威力が前回の2倍'
      + '（最大128倍まで）。多段は一切出ない',
  },
  {
  ...
  /* ===== 弱点コンボを伸ばす (§5.7) =====
   * 弱点コンボ (§10.6) は属性有利か弱体中の相手を殴ると積む。
   * **オートでも積む**（battle.js の updateCombo に手動/オートの分岐は無い）。
   * 実測でもオート5連戦で2〜3段まで伸びる。
   *
   * 手動が有利なのは「積める／積めない」ではなく、
   * 弱体を入れてから殴るような**段取りを組める**こと。
   * ここを厚くするほど、その段取りの見返りが大きくなる。 */
  {
  ...
  {
    // ── 段の出口を「威力以外」へ広げる枝 (§5.10) ──
    //
    // 段を使う技はこれ以前5本あり、**全部が攻撃技**だった。どんな消費の仕方をしても
    // 行き先が威力なので、支援役とタンクには段を使う理由が一つも無かった。
    //
    // しかもクラス技は解禁2〜4・再使用2〜6ラウンドなのに、推奨レベルの戦闘は
    // 実測 1.0〜3.5 ラウンドしか続かない。**解禁3以上の6本は一度も撃てず、
    // どの技も2回目が来ない。** 33本のクラス技の層がほぼ死んでいた。
    //
    // 段は battle.combo.count でパーティ共有なので、積むのはアタッカー、
    // 使うのは支援、という関係が生まれる。支援の枝として上級に置いた。
    id: 'tr_grant_reprise', tier: 'high', name: '継ぎ手の型', cost: 5, maxLevel: 1,
    effects: [{ kind: 'grant_skill', skill: 'sk_combo_reprise', value: 1 }],
    desc: 'アクティブ技「継ぎ手」を習得（段2を消費し、味方全員のクールタイムを進めて再行動）',
  },

  {
    // 染色 (§9.1)。**受け側の属性に触る唯一の手。**
    // 攻撃側の属性操作はツリーが埋め尽くしているのに、ここは空いていた。
    //
    // 属性の枝ではなく支援の枝に置く。染めた本人ではなく、
    // **その相手を殴る味方全員**が得をするので、マークと同じ立ち位置になる。
    id: 'tr_grant_dye', tier: 'mid', name: '色差しの型', cost: 3, maxLevel: 1,
    effects: [{ kind: 'grant_skill', skill: 'sk_tree_dye', value: 1 }],
    desc: 'アクティブ技「色を差す」を習得（敵1体を自分の属性に染める）',
  },

  {
    // 段の刻み（レシート型）(§5.10)。継ぎ手と同じ「支援が段を使う」枝だが、
    // **溜める判断そのものを問う**のが違い。継ぎ手は2段で固定なので、
    // 段が余っていても使い道が変わらない。
    //
    // 継ぎ手より安い（4点）のは、届く行が段数しだいで、
    // 2段しか無いときは障壁1行しか出ないため。上の行まで届かせるには
    // 段を積む側（アタッカー）との噛み合わせが要る。
    id: 'tr_grant_ledger', tier: 'high', name: '積み上げの型', cost: 4, maxLevel: 1,
    effects: [{ kind: 'grant_skill', skill: 'sk_combo_ledger', value: 1 }],
    desc: 'アクティブ技「積み上げの証」を習得（段を全部使い、届いた刻みぶん効果が重なる）',
  },
  ...
  /* ── 属性と状態異常の【極】 (§5.13) ──
   *
   * 【極】は「上級帯の一角にしか効かない」と作業ログに書いてあったが、
   * その後 支援・回復側に5つ足されている。**残っていたのはこの2軸。**
   *
   * どちらも代償は「その軸の外側」に置いてある。軸の内側を削ると、
   * 同じ軸の別の枝で買い戻せてしまい、実質無料の強化になる
   * （支援の【極】が遮断フラグを使っているのと同じ理由）。
   */
  {
    // 相克。有利を取り切る代わりに、突かれる側にも回る。
    //
    // ── 代償の置き場所を一度間違えた ──
    // 最初は「全属性で受けるダメージ +45%」だけにしていたが、
    // element: 'all' は **無属性を含まない**（tree.js の ELEMENTS）。
    // 敵の30%は無属性で、しかも終わりなき回廊は**4種すべてが無属性**。
    // つまり一番周回される場所で代償がまるごと効かず、ただの無料強化になっていた。
    // 実測して気付いた。
    //
    // 次に素のDEFへ移したが、**これも失敗**。装備ぶんが大半を占めるので
    // 素のDEFを80%削っても合計では -3.6% にしかならず、
    // 有利時 +112% の対価として話にならなかった（【極】一閃の注記と同じ罠）。
    //
    // 既存の【極】に倣い、**火力側で払わせる**形にした。
    // 等倍の相手への火力を落とす。有利を作れない相手には弱くなるので、
    // 相性を組み立てられる編成でなければ払い損になる。
    // 加えて最大HPも削る。属性適応で「常に有利」にして等倍の代償を
    // 帳消しにする組み方があるため、**それでも残る代償**を1つ持たせる。
    //
    // 属性適応・変換・貫通はどれも**自分の攻撃側**にしか効かないので、
    // どちらの代償も同じ軸では買い戻せない（計画で挙がっていた落とし穴）。
    //
    // 有利倍率は 1.5 → 2.0（極意3段と合わせると 2.3）。
    // 等倍・不利の相手には**何も足さない**ので、
    // 相性を作れる編成でなければ払い損になる。
    id: 'tr_x_clash', tier: 'high', name: '【極】相克', cost: 10, maxLevel: 1,
    effects: [
  ...
  },
  {
    ...
    //
    // 代償を素のATK・魔力に置いた理由:
    // **継続ダメージは相手の最大HPを見て決まる**ので、こちらの攻撃力を削っても
    // 撒く量は減らない。つまり「弱体で削る型」の身入りは保ったまま、
    // 「殴りのついでに毒も乗せる」つまみ食いだけが割に合わなくなる。
    // 状態異常の割合ダメージは、投資効率が良すぎるつまみ食いが以前から懸念だった。
    //
    // ── なぜ「継続ダメージ +45%」に意味があるのか（実測） ──
    // status_power を持つ枝は tr_status_power の**1本だけ**で、
    // 2SP×5段＝10SPで上限（+100%）に届く。そこから先は何段積んでも増えない。
    //
    //   未投資      毒 1.50% / 1回
    //   つまみ食い20SP 毒 3.00%   ← ここで既に頭打ち
    //   専門    62SP 毒 3.00%   ← 42SP 足しても**1ミリも増えない**
    //   専門+【極】疫  毒 3.67%
    //
    // つまりこの軸は「撒く量」に関しては深さが無く、
    // **つまみ食いと専門家が同じ火力**という状態だった。
    // この【極】は、専門家だけがそこから先へ進める唯一の段になる。
    //
    // 安全弁（STATUS_CAP の毒 0.10）にはまだ遠い。上限で無駄になってはいない。
    id: 'tr_x_plague', tier: 'high', name: '【極】疫', cost: 10, maxLevel: 1,
    effects: [
