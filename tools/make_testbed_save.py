...
── ⚠ 終盤の難度には使えない ──
`autoequip.optimize` はセットを組まないので、**装備セットの効果が丸ごと抜ける**。
実測: 主人公の軽減が 60.7%（実セーブ・set_undying 発動）に対し 44.2%、
多段も 1 しか付かない（実セーブは3）。ダメージ上限は1発ごとにかかるので、
発数の差がそのまま天井の差になる。

結果として **ATK は高いのに勝てない**という逆転が起きる。
q_endless_vigil と q_three_at_the_verge はこの土台で 0%、実セーブで 100%。
**土台の0%を「達成不能」と読まないこと。** 詳しくは docs/測定エージェントの手順.md。
